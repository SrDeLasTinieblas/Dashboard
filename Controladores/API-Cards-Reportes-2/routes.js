...
routes.get('/:SumarPrecioVenta', (req, res)=>{
    req.getConnection((err, conn)=>{
        if(err) return res.send(err)
        conn.query('SELECT SUM(Precio) SUMValor FROM orden_detalles',
         (err, rows)=>{
            if(err) return res.send(err)
...
/*routes.put('/:update', (req, res)=>{
    req.getConnection((err, conn)=>{
        if(err) return res.send(err)
        conn.query('UPDATE `todo` SET `slider`= ?  WHERE id = 1; ',[req.body, req.params.update], (err, rows)=>{
            if(err) return res.send(err)
